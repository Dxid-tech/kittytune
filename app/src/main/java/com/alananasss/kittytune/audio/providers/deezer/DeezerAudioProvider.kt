...
private object Timber {
    ...
    class Logger(val tag: String) {
        ...
        fun d(t: Throwable?, message: String) { Log.d(tag, message, t) }
        fun w(message: String) { Log.w(tag, message) }
        fun w(t: Throwable?, message: String) { Log.w(tag, message, t) }
        fun i(message: String) { Log.i(tag, message) }
        fun e(message: String) { Log.e(tag, message) }
        fun e(t: Throwable?, message: String) { Log.e(tag, message, t) }
    }
}
...
object DeezerAudioProvider {
    ...
    suspend fun downloadDecryptedStream(
        ...
        val request = DeezerAudioDataSource.RequestParams.fromUri(uri)
        val requestBuilder = Request.Builder()
            .url(wrapMediaUrlForProxy(request.mediaUrl, request.proxyUrl))
            .get()
            .header("User-Agent", BROWSER_USER_AGENT)
        val requestClient = clientForProxy(client, request.proxyUrl)
        val response = requestClient.newCall(requestBuilder.build()).execute()
        if (!response.isSuccessful) {
            response.close()
            throw IOException("Deezer stream HTTP ${response.code}")
        }
        val body = response.body ?: throw IOException("Empty Deezer response body")
        val totalLength = request.contentLength ?: body.contentLength().takeIf { it > 0L } ?: -1L
    ...
    companion object {
        ...
        fun buildUri(
            ...
            format?.takeIf { it.isNotBlank() }?.let {
                builder.appendQueryParameter("format", it)
            }
